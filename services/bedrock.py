...
# Nova Pro for chat/brainstorming — smarter, on-demand, no payment issues
CHAT_MODEL = os.getenv("BEDROCK_MODEL_ID", "amazon.nova-pro-v1:0")

# Nova Lite for doc generation — faster, cheaper, still great for structured output
...
def call_bedrock(system_prompt, messages, tool_config=None, use_docs_model=False):
    """
    use_docs_model=False (default) → Nova Pro   — chat & brainstorming
    use_docs_model=True            → Nova Lite  — doc generation (cheaper)
    Falls back to OpenRouter if Bedrock fails.
    """
    errors = []
    model_id = DOCS_MODEL if use_docs_model else CHAT_MODEL
    label = "Nova Lite (docs)" if use_docs_model else "Nova Pro (chat)"

    # PRIMARY: Bedrock Nova
    try:
        ...
        errors.append(f"Bedrock {label}: {e}")
